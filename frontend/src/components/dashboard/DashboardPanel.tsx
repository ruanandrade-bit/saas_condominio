...
const DashboardPanel = ({ title, subtitle, icon, children }: DashboardPanelProps) => (
  <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-soft">
    <header className="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-50 text-slate-600 ring-1 ring-slate-200/80">
        {icon}
      </div>
      <div className="min-w-0">
        <h3 className="truncate text-sm font-extrabold tracking-[-0.02em] text-slate-950">{title}</h3>
